    ...
    <script>
        ...
        function setDataRow(item, number) {
            ...
            row.appendChild(qingjiaNumberCell);
                ...
                //请假原因
            var qingjiaContentCell = document.createElement('td');
            //如果图文详情包含图片
            if (item.qingjiaContent && item.qingjiaContent.indexOf('img') != -1) {
                //暂时只考虑图片+文字 和 文字+图片的情况
                var beginIndex = item.qingjiaContent.indexOf('<img');
                var endIndex = item.qingjiaContent.indexOf('>');
                    qingjiaContentCell.innerHTML = item.qingjiaContent.substring(beginIndex, endIndex + 1).replace("img", "img width='100' height='100'");
            } else {
                if (item.qingjiaContent != null && item.qingjiaContent != "" && item.qingjiaContent.length >= 11) {
                        qingjiaContentCell.innerHTML = item.qingjiaContent.substring(0, 10) + "...";
                } else {
                        qingjiaContentCell.innerHTML = item.qingjiaContent;
                }
            }
            row.appendChild(qingjiaContentCell);
        ...
        function shenhe() {
            ...
            if (mymessage == true) {
                http("qingjia/shenheqingjia?shenhe="+$("#shenhe").val()+"&ids="+$("#ids").val(), "get", {}, (res) => {
                    if(res.code == 0)
                {
                    getDataList();
                    alert('操作成功');
